...
way to "force-on" audit when it's disabled at the master switch.
...
The file is opened with `flags: 'a'` (append-only) and stays open for
the process lifetime. On `SIGTERM` the transport flushes pending writes
before exiting (`auditSink.shutdown()` is called from
...
Use `logrotate` (Linux) or your container platform's native log driver:
...
Plan 8 Phase E shipped an OTLP audit sink stub that currently routes
to stderr with a `[FALLBACK:logs-pipeline-not-wired]` prefix. This is
intentional: enabling `MCP_AUDIT_SINK=otlp` should not silently drop
audit events when the LoggerProvider is not yet wired.

The full OTLP path requires:

1. `@opentelemetry/api-logs` LoggerProvider boot in
   `mcp-server/src/otel.ts`.
2. A logs pipeline (BatchLogRecordProcessor → OTLPLogExporter) wired
   to the same OTLP endpoint as traces.
3. `OtlpAuditSink` swapped from stderr-fallback to `logger.emit(...)`.

This is tracked but out of scope for v0.8.x.

---

## Compliance considerations (SOC2 / SOX / GDPR)

### Idempotent tools are NOT audited

By design (Plan 8 §10 critical rule 2), the audit middleware
short-circuits when `tool.idempotent === true`. This covers all
...
reads. Read patterns are still observable via telemetry (every tool
...
Sensitive arg fields are redacted in `args_redacted` before hitting
the sink. See `redact.ts` for the full policy:

- **Global keys**: `token`, `bearer_token`, `auth`, `password`,
  `secret` redacted at any depth (case-insensitive).
- **Per-tool keys**: 15 mutating tools have explicit content-bearing
  fields (`content`, `embeds`, `components`, `messages`, etc.)
  marked sensitive. New tools must opt in via the
  `SENSITIVE_KEYS_BY_TOOL` map.
- **Length-aware marker**: redacted strings become
  `[REDACTED:${length}ch]` so the log preserves a size signal
...
Every event written to the sink is a single JSON object on one line.
Fields:
