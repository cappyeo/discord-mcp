...
export const CURSOR_CLI_ACTIVATION_CONFIRMATION_PREFIX = 'APPROVE_CURSOR_CLI_ACTIVATION:';
export const CURSOR_CLI_ACTIVATION_WRITE_CONFIRMATION_PREFIX =
  'APPROVE_CURSOR_CLI_ACTIVATION_WRITE:';
export const CURSOR_CLI_ACTIVATION_MAX_DURATION_MS = 600_000;
export const CURSOR_CLI_ACTIVATION_PHASE_TIMEOUT_MS = 180_000;
export const CURSOR_CLI_ACTIVATION_RECOVERY_TIMEOUT_MS = 30_000;
export const CURSOR_CLI_ACTIVATION_CANCELLATION_TIMEOUT_MS = 5_000;
...
const CURSOR_API_KEY_MAX_BYTES = 4096;
...
function expectedConfirmation(release, trialId) {
  return `${CURSOR_CLI_ACTIVATION_CONFIRMATION_PREFIX}${release}:${trialId}`;
}

function expectedWriteConfirmation(release, trialId) {
  return `${CURSOR_CLI_ACTIVATION_WRITE_CONFIRMATION_PREFIX}${release}:${trialId}`;
}

function validateCursorApiKey(value) {
  if (typeof value !== 'string' || value.trim() === '' || /[\r\n]/u.test(value))
    throw new Error('CURSOR_API_KEY preflight failed');
  if (Buffer.byteLength(value, 'utf8') > CURSOR_API_KEY_MAX_BYTES)
    throw new Error('CURSOR_API_KEY preflight failed');
  return value;
}

/** Validate Cursor host authentication before campaign-local side effects. */
export function assertCursorCliActivationAuthReady(environment = process.env) {
  validateCursorApiKey(environment?.CURSOR_API_KEY);
  return true;
}
...
function assertSecretFree(value, token, label) {
  ...
  if (/DISCORD_TOKEN|CURSOR_API_KEY/iu.test(text))
...
export function validateCursorCliActivationRequest(options = {}) {
  if (!record(options)) throw new TypeError('Cursor activation request must be an object');
  ...
  if (!record(options.target)) throw new TypeError('Cursor controlled target is required');
  ...
  if (options.target.controlled !== true)
    throw new TypeError('Cursor controlled target confirmation is required');
  if (options.target.callerOwned !== true)
    throw new TypeError('Cursor caller-owned bot confirmation is required');
  ...
  if (confirmation !== expectedConfirmation(options.release, options.trialId))
    throw new TypeError('Cursor operator confirmation does not match release and trial');
  ...
  if (
    ...
    throw new TypeError('Cursor write approval does not match release and trial');
  if (options.maxDurationMs !== undefined) {
    if (
      !Number.isSafeInteger(options.maxDurationMs) ||
      options.maxDurationMs <= 0 ||
      options.maxDurationMs > CURSOR_CLI_ACTIVATION_MAX_DURATION_MS
    )
  ...
  return {
    ...
    maxDurationMs: options.maxDurationMs ?? CURSOR_CLI_ACTIVATION_MAX_DURATION_MS,
    token,
    executionMode,
  };
}

export function buildCursorCliSetupArgs({ profile, guildId, configPath }) {
  ...
  if (typeof configPath !== 'string' || !isAbsolute(configPath))
    throw new TypeError('Cursor configPath must be absolute');
  return [
    'setup',
    '--profile',
    profile,
...
export function parseCursorCliSetupJson(stdout, target) {
  if (typeof stdout !== 'string' || stdout.trim() === '')
    throw new Error('Cursor guided setup did not emit JSON output');
  let output;
  try {
    output = JSON.parse(stdout);
  } catch {
    throw new Error('Cursor guided setup emitted invalid JSON output');
  }
  if (!record(output) || output.ok !== true || (output.exitCode ?? 0) !== 0)
    throw new Error('Cursor guided setup JSON result was not successful');
  ...
  if (
    ...
    throw new Error('Cursor guided setup bot binding is invalid');
  if (
    !Array.isArray(allowedGuilds) ||
    allowedGuilds.length !== 1 ||
    allowedGuilds[0] !== target.guildId
  )
    throw new Error('Cursor guided setup guild binding is invalid');
  return {
...
export function assertCursorCliConfigReady(config, { release, token, profile } = {}) {
  const parsed = parseConfig(config, 'Cursor generated config');
  assertSecretFree(parsed, token, 'Cursor generated config');
  ...
  if (
    ...
    throw new Error('Cursor generated config is not pinned to the release and profile');
  return true;
}

function canonicalAllowConfig({ request, workspaceState, install }) {
  if (!record(request) || !record(workspaceState) || !record(install))
    throw new TypeError('Cursor canonical config context is required');
  if (!DIGEST.test(install.cliDigest ?? ''))
    throw new Error('Cursor install provenance is unavailable');
...
export function assertCursorCliConfigWritable(config, context = {}) {
  const parsed = parseConfig(config, 'Cursor canonical config');
  assertSecretFree(parsed, context.request?.token, 'Cursor canonical config');
  const expected = canonicalAllowConfig(context);
  if (JSON.stringify(parsed) !== JSON.stringify(expected))
    throw new Error('Cursor canonical config is not exactly target-bound and write-enabled');
  return true;
}
...
const defaultLiveAdapter = ({ environment, verifyRuntimePackage }) =>
  createCursorCliActivationLiveAdapter({ environment, verifyRuntimePackage });

export function createDefaultCursorCliActivationDependencies(options = {}) {
  if (!record(options)) throw new TypeError('Cursor dependency options must be an object');
  const prototype = Object.getPrototypeOf(options);
  if (prototype !== Object.prototype && prototype !== null)
    throw new TypeError('Cursor dependency options must not inherit activation seams');
  ...
  const dependencies = createActivationDependencies({
    ...
    resolveNpmCli,
    environment,
    setupArgs: buildCursorCliSetupArgs,
    parseSetup: parseCursorCliSetupJson,
    assertConfigReady: assertCursorCliConfigReady,
    assertConfigWritable: assertCursorCliConfigWritable,
    ...
    executionProvenance: {
      execution_mode: 'live',
      adapter_id: 'discord-mcp.cursor-cli-activation.v1',
      abortable: true,
...
export async function runCursorCliActivationTrial(options = {}) {
  const request = validateCursorCliActivationRequest(options);
  const dependencies = options.dependencies ?? createDefaultCursorCliActivationDependencies();
  ...
  return runActivationTrial({
    ...
    host: 'cursor-cli',
    limits: {
      phaseTimeoutMs: CURSOR_CLI_ACTIVATION_PHASE_TIMEOUT_MS,
      recoveryTimeoutMs: CURSOR_CLI_ACTIVATION_RECOVERY_TIMEOUT_MS,
      cancellationTimeoutMs: CURSOR_CLI_ACTIVATION_CANCELLATION_TIMEOUT_MS,
    },
    assertConfigReady: assertCursorCliConfigReady,
    assertConfigWritable: assertCursorCliConfigWritable,
...
export function parseCursorCliActivationArgs(argv) {
  if (!Array.isArray(argv)) throw new TypeError('Cursor activation arguments must be an array');
  const values = {};
  for (let index = 0; index < argv.length; index += 1) {
    const flag = argv[index];
    if (!ACTIVATION_CLI_FLAGS.has(flag) || Object.hasOwn(values, flag))
      throw new TypeError('invalid Cursor activation arguments');
  ...
  for (const flag of ACTIVATION_CLI_FLAGS)
    if (!Object.hasOwn(values, flag)) throw new TypeError('invalid Cursor activation arguments');
...
function activationCliToken(environment) {
  ...
  if (!token) throw new Error('Cursor activation environment is incomplete');
  return normalizeDiscordToken(token);
}

function activationCliFailure() {
  return {
    schema_version: 'discord-mcp.cursor-cli-activation-cli.v1',
    ok: false,
    error: 'Cursor activation trial failed',
  };
}

function writeSecretFreeCliResult(stdout, result, token) {
  const serialized = JSON.stringify(result);
  if (serialized.includes(token) || /CURSOR_API_KEY/iu.test(serialized)) {
...
export async function main({
  ...
  try {
    const options = parseCursorCliActivationArgs(argv);
