...
import {
  GrokCliConfigError,
  inspectGrokCliConfig,
} from '../lib/client-snippets/grok-cli-inspector.js';
...
function grokCliClientConfigCheck(profile: DiscordMcpProfile, config?: string): CheckResult {
  try {
    const inspection = inspectGrokCliConfig(profile, {
      ...(config === undefined ? {} : { config }),
    });
    return {
      id: 'grok-cli-client-config',
      status: 'ok',
      message: `Configured Grok Build launcher is secret-safe for profile ${profile.name}.`,
      details: {
        profile: profile.name,
        audited: true,
        server: inspection.configName,
    ...
    const persisted = error instanceof GrokCliConfigError && error.kind === 'credential-persisted';
    return {
      id: 'grok-cli-client-config',
      status: 'fail',
      message: persisted
        ? 'Grok Build MCP config contains a credential. Remove it, keep DISCORD_TOKEN in the launch environment, keep XAI_API_KEY or login state at the host level, and rotate any materialized credential.'
        : 'Could not verify a secret-safe generated Grok Build launcher.',
      details: {
        profile: profile.name,
        audited: false,
        credentialPersisted: persisted ? true : null,
...
export async function doctorAction(opts: DoctorOptions): Promise<void> {
    ...
    emitResult(
      {
        ...
        errors: [
          'Supported client audits: codex, antigravity-cli, cursor-cli, gemini-cli, grok-cli.',
        ],
      },
      opts.json === true,
    );
  ...
  if (opts.client === 'grok-cli' && profile !== undefined) {
    results.push(grokCliClientConfigCheck(profile, opts.config));
  }
