...
import emojisDelete from './delete.js';
import '../../container.js';

const DISCORD_API = 'https://discord.com/api/v10';

describe('emojis_delete', () => {
  it('DELETEs the emoji and returns deleted:true', async () => {
    container.rest = new REST({ version: '10', makeRequest: fetch }).setToken('fake-token-aaaaaa');
    server.use(
      http.delete(
        `${DISCORD_API}/guilds/:guildId/emojis/:emojiId`,
        async () => new HttpResponse(null, { status: 204 }),
      ),
    );
    const T = emojisDelete;
    const t = new T(
      { name: 'emojis_delete', path: 'inline', root: 'inline', store: null as never },
      { name: 'emojis_delete', enabled: true },
    );
    const r = (await t.run(
