...
import stageInstancesDelete from './delete.js';
import '../../container.js';

const DISCORD_API = 'https://discord.com/api/v10';

describe('stage_instances_delete', () => {
  it('DELETEs /stage-instances/:cid and returns deleted:true', async () => {
    container.rest = new REST({ version: '10', makeRequest: fetch }).setToken('fake-token-aaaaaa');
    server.use(
      http.delete(
        `${DISCORD_API}/stage-instances/:cid`,
        async () => new HttpResponse(null, { status: 204 }),
      ),
    );
    const T = stageInstancesDelete;
    const t = new T(
      { name: 'stage_instances_delete', path: 'inline', root: 'inline', store: null as never },
      { name: 'stage_instances_delete', enabled: true },
    );
    const r = (await t.run(
