import { server } from '@cappyeo/discord-mcp-server-mocks';
import { REST } from '@discordjs/rest';
import { container } from '@sapphire/pieces';
import { HttpResponse, http } from 'msw';
import { describe, expect, it } from 'vitest';
import threadsAddMember from './add_member.js';
import '../../container.js';

const DISCORD_API = 'https://discord.com/api/v10';

describe('threads_add_member', () => {
  it('PUTs thread-members/:userId and returns added:true', async () => {
    container.rest = new REST({ version: '10', makeRequest: fetch }).setToken('fake-token-aaaaaa');
    server.use(
      http.put(
        `${DISCORD_API}/channels/:threadId/thread-members/:userId`,
        async () => new HttpResponse(null, { status: 204 }),
      ),
    );
    const T = threadsAddMember;
    const t = new T(
      { name: 'threads_add_member', path: 'inline', root: 'inline', store: null as never },
      { name: 'threads_add_member', enabled: true },
    );
    const r = (await t.run(
      { thread_id: '999000999000999301', user_id: '111111111111111111' },
      { signal: new AbortController().signal },
    )) as { isError: boolean; structuredContent: { added: boolean } };
    expect(r.isError).toBe(false);
    expect(r.structuredContent.added).toBe(true);
  });
});
