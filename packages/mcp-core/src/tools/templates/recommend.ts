...
import { TemplateProvenanceSchema, templateProvenance } from './_lib/provenance.js';
...
const RecommendationOutputShape = {
  ...
  rejected_candidates: z.array(RejectedCandidateSchema),
...
interface LiveInspection {
  ...
  readonly provenance: ReturnType<typeof templateProvenance>;
}
...
async function inspectLiveTemplate(
  ...
  const provenance = templateProvenance(raw);
  return {
    evidence: {
      code: candidateCode,
      verified: true,
...
function candidateOutput(
  ...
  if (code === null || blueprint === undefined || provenance === undefined) {
...
export async function recommendTemplates(
  ...
  return {
    ...
    data: {
      ...
      rejected_candidates: portfolio.rejected,
