import { server } from '@cappyeo/discord-mcp-server-mocks';
import { REST } from '@discordjs/rest';
import { container } from '@sapphire/pieces';
import { HttpResponse, http } from 'msw';
import { describe, expect, it } from 'vitest';
import membersRemoveRole from './remove_role.js';
import '../../container.js';

const DISCORD_API = 'https://discord.com/api/v10';

describe('members_remove_role', () => {
  it('DELETEs /guilds/:gid/members/:uid/roles/:rid and returns removed:true', async () => {
    container.rest = new REST({ version: '10', makeRequest: fetch }).setToken('fake-token-aaaaaa');
    server.use(
      http.delete(
        `${DISCORD_API}/guilds/:gid/members/:uid/roles/:rid`,
        async () => new HttpResponse(null, { status: 204 }),
      ),
    );
    const T = membersRemoveRole;
    const t = new T(
      { name: 'members_remove_role', path: 'inline', root: 'inline', store: null as never },
      { name: 'members_remove_role', enabled: true },
    );
    const r = (await t.run(
      {
        guild_id: '999000999000999000',
        user_id: '111122223333444455',
        role_id: '222233334444555566',
      },
      { signal: new AbortController().signal },
    )) as { isError: boolean; structuredContent: { removed: boolean } };
    expect(r.isError).toBe(false);
    expect(r.structuredContent.removed).toBe(true);
  });
});
