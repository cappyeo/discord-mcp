...
describe('ALLOWED_GUILDS server boundary', () => {
  it('preserves the full 201-tool compatibility surface when unset', async () => {
    const { client } = await connect();
    expect((await client.listTools()).tools).toHaveLength(201);
  });

  it('hides unprovable routes and advertises the remaining 179 tools', async () => {
