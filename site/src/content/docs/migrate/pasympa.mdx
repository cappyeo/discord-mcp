---
title: From PaSympa
description: Migrate from PaSympa's discord_* tool surface (~91 tools) onto discord-mcp's 205-tool catalog.
sidebar:
  order: 1
---
...
- **Adapter source**:
  [`packages/mcp-server/src/lib/migrate-adapters/pasympa.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-server/src/lib/migrate-adapters/pasympa.ts)

## Why migrate to discord-mcp

- **Production hardening** - first-class OpenTelemetry traces and Cockatiel
  resilience policies (retry / circuit breaker / bulkhead) on the central
  Discord REST path.
...
- **Broad Discord surface** - 198 Discord tools cover stickers, automod, polls, application
...
- **Gateway event notifications** - `resources/subscribe` can signal that
  Discord state changed; fetch current state with the corresponding REST tool.
  It complements polling but does not replace the read step. See
...
5. **Generate your new client config.**
   ...
   The 198 Discord-operation tools replace PaSympa's 91; the separate
   `inspiration` category remains optional.

</Steps>

## Representative mappings

The table below shows common PaSympa operations and their discord-mcp
equivalents. See the
[adapter source](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-server/src/lib/migrate-adapters/pasympa.ts)
...
- [discord-mcp tools (205)](/discord-mcp/tools/) - auto-generated reference.
- [CLI reference: `migrate`](/discord-mcp/reference/cli/#migrate) - flags
  and exit codes.
- [Architecture: gateway](/discord-mcp/architecture/gateway/) - how
  notifications and authoritative REST reads work together.
