...
- **Broad Discord surface** - discord-mcp covers 198 Discord tools, plus an
  optional external inspiration tool, about
...
**discord-mcp**: Ships 209 tools and filters the advertised registry server-side
with `MCP_CATEGORIES`; the optional `inspiration` category is the only
third-party discovery surface.
...
**discord-mcp**: `MCP_DRY_RUN=true` protects the 31 tools registered with
`confirm_required` plus the payload-bound Components V2 send/edit/template
policy. Other ordinary writes-including many send and create tools-can still
