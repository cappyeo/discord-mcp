---
title: Components V2 — build a rich announcement
...
<Aside type="caution">
`components_v2_send` is an ordinary write and is not confirmation-gated. It validates locally,
then posts immediately. Components V2 is irreversible for that message; it cannot later become
a plain-content message.
</Aside>
...
5. **Send the same validated array.**
   ...
   The result contains `message_id`, `channel_id`, `jump_url`, and
   `component_count: 1` because there is one top-level container.
