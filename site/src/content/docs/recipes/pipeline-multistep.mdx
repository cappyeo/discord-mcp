---
title: Chain three calls
description: Use mcp_pipeline to compose announce, schedule, and grant-role in one fail-fast server-side request.
sidebar:
  order: 3
---
...
1. **Build the pipeline payload.**
   ...
   announcement step's `message_id` flows into the event's description. The
   host's user ID and the role ID are known workflow inputs and are supplied
   directly to the role-grant step.
...
2. **Read the response.**
   ...
   <Tabs>
     <TabItem label="Output (success)">
       ```json
       {
         "steps": [
           {
             ...
             "result": {
              ...
              "timestamp": "2030-01-01T16:30:00.000Z"
             },
             "duration_ms": 142
           },
           {
             ...
             "status": "success",
             "result": {
               "id": "333344445555666677",
               "guild_id": "111122223333444455",
              "name": "Office hours - January 1",
               "description": "Linked to announcement 888899990000111122",
              "scheduled_start_time": "2030-01-01T17:00:00.000Z",
              "scheduled_end_time": "2030-01-01T18:00:00.000Z",
               "status": 1,
               "entity_type": 3,
               "channel_id": null
             },
         ...
         "variables": {
           ...
           "event": { "id": "333344445555666677", "name": "Office hours - May 1" },
           "grant": { "added": true, "user_id": "777788889999000011", "role_id": "555566667777888899" }
         },
     ...
     <TabItem label="Output (early abort)">
       ```json
       {
         "steps": [
           { "id": "announce", "tool": "messages_send", "status": "success", "duration_ms": 142 },
           {
             ...
             "tool": "events_create",
             "status": "error",
             "error": { "code": "DISCORD_PERMISSION_DENIED", "message": "Missing MANAGE_EVENTS", "retriable": false },
             "duration_ms": 88
           }
         ],
       ...
       Note that `grant` is **absent**, not `"skipped"`. On an abort the
       executor breaks out of the loop, so steps after the failing one are
       never dispatched and never appear in `steps`. `"skipped"` is emitted
       only for a step whose `if` condition evaluated falsy.
     </TabItem>
   </Tabs>
...
- **Default abort-on-error.** When a step returns an error, the pipeline sets
  `aborted: true` and stops immediately. Subsequent steps are not dispatched
  and do **not** appear in `steps` - the array simply ends at the failing step.
  The agent sees exactly which step blew up and what the partial state is.
...
| Audit logging | Depends on each tool's read-only annotation | One parent event plus events for every non-read-only leaf; explicitly read-only leaves are skipped |

<Aside type="caution">
Stopping is not rollback. Any Discord side effect from an earlier successful step remains and
must be compensated explicitly if your workflow requires reversal.
</Aside>
