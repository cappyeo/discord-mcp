---
title: Summarize a busy channel
description: Summarize a Discord channel through MCP sampling, with an explicit raw-message fallback when the client cannot sample.
sidebar:
  order: 4
---
...
<Aside type="note">
**Sampling capability is negotiated at initialization.** Do not infer support
from a client name or version. When `sampling` is absent, the tool returns raw
messages plus `_meta.fallback: 'host_llm_should_process'`; the host model must
produce the summary in a later turn. The fallback result is data to summarize,
not a completed summary.
</Aside>
...
3. **Post the summary.**
   ...
   instead read the top-level `raw_messages`, generate the summary itself in the next
...
- **Explicit degradation.** Clients without sampling receive the source data
  and a machine-readable fallback marker. The host can then summarize it in a
  separate model turn.
