---
title: Client setup
description: Wire up discord-mcp with Claude Desktop, Claude Code, Codex, Antigravity CLI, Cursor Agent CLI, legacy Gemini CLI, the Cursor editor, or any generic MCP client.
type: tutorial
sidebar:
  order: 22
prev:
...
profile. The Codex fragment forwards `DISCORD_TOKEN` from the environment. Antigravity CLI, Cursor Agent CLI, and most
JSON fragments use the environment inherited by the client process and omit a token field. Gemini CLI instead emits
