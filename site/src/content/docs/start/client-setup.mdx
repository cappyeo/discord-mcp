...
The client should show **4 visible tools**: `mcp_tools_search`, `mcp_tools_read`,
`mcp_tools_write`, and `mcp_tools_destructive`. This is the expected working state, not a
partial installation. The client searches for an exact Discord tool, then invokes it through
the risk-matched dispatcher.
