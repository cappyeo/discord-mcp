...
discord-mcp ships as a single npm package (`@cappyeo/discord-mcp`) that bundles the server.
...
```bash
pnpm add -g @cappyeo/discord-mcp
```

## Install with yarn

```bash
yarn global add @cappyeo/discord-mcp
```
...
```bash
npm install -g @cappyeo/discord-mcp discord.js
