...
The JSON result uses schema `discord-mcp.catalog-check.v1` and reports 209 tools,
...
the ordinary catalog remains static unless an allowlisted guild is configured;
`discord://guild/{id}/info` is then listed/read as a short-lived live snapshot,
while the other event URIs remain notification-only.
