...
| `doctor` | Diagnose token/connectivity state and audit generated Codex or Gemini CLI configuration with `--client`. |
| `smoke` | Verify the real MCP-to-Discord path; writes require the explicit `--confirm-write` flag. |
| `init` | Print an MCP client config snippet for Claude Desktop / Claude Code / Codex / Gemini CLI / Cursor / generic. |
