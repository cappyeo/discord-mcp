...
This command requires no Discord token and performs no Discord request or write. It is
catalog validation only; it does not count as Activity Evidence. Continue to
[guided setup](/discord-mcp/start/) when you are ready to connect a caller-owned bot.

The JSON result uses schema `discord-mcp.catalog-check.v1` and reports 208 tools,
6 static resources, a `CATALOG_ONLY` execution guard, `discord_execution: "disabled"`,
and `activity_evidence_created: false`. It validates the installed catalog contract;
...
| `doctor` | Diagnose token/connectivity state and audit generated Codex, Antigravity CLI, Cursor Agent CLI, or Gemini compatibility configuration with `--client`. |
| `smoke` | Verify the real MCP-to-Discord path; writes require the explicit `--confirm-write` flag. |
| `init` | Print an MCP client config snippet for Claude Desktop / Claude Code / Codex / Antigravity CLI / Cursor Agent CLI / Gemini compatibility / Cursor editor / generic. |
