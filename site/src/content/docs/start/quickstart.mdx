...
2. **Generate your client config snippet and merge it by hand.**
   ...
   The generated `env` block uses `${env:DISCORD_TOKEN}` by default, keeping the
   secret out of the printed snippet. Leave it as-is when your client supports
   environment interpolation; otherwise replace it only in your local client
   config and never commit that config file.
