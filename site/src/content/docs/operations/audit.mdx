...
The audit middleware emits an event for tools whose metadata declares
`idempotent: false`. This is not the same as a complete write audit: some
Discord writes are idempotent and are skipped by the current predicate. Use
the trail as an operational aid, not proof that every mutation was captured.
...
The audit middleware short-circuits when `tool.idempotent === true`. This
covers read-only operations, but it also excludes idempotent writes. The
current catalog contains write tools that therefore do not emit an event.
...
currently no flag to include reads or idempotent writes. If your controls
