...
`elicitation` — the last of which discord-mcp does not use today. Not every
client implements all five. This page is the
...
| `elicitation` | yes | yes | no | rare | Mid-tool human-in-the-loop confirmation. **Not used by discord-mcp today** — the server never elicits, and every gated tool goes through the `__confirm` contract regardless of client support. |
...
- **Known limitations**: no `resources/subscribe`. The missing `elicitation`
  support costs nothing here — discord-mcp never elicits, so destructive
  tools use the `__confirm:true` contract on every client (see
...
- [Architecture → Confirmation](/discord-mcp/architecture/confirmation/) — the `__confirm` contract that gates destructive tools on every client.
