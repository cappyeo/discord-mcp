---
title: Resilience
description: discord-mcp's composite resilience policy — bulkhead, circuit breaker, retry, timeout. 10 environment variables; 429-aware backoff; pipeline interactions.
sidebar:
  order: 2
---
...
[`packages/mcp-core/src/rest/policy.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/rest/policy.ts).
...
- **Long-running bulk operations** (e.g. bulk-ban, archive sweep):
  consider raising `MCP_TIMEOUT_DEFAULT_MS` rather than retry count — the
...
- Source: [`packages/mcp-core/src/rest/policy.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/rest/policy.ts), [`resilient.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/rest/resilient.ts).
