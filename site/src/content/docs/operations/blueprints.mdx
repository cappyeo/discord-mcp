...
1. On the recommended progressive surface, call `build_discord_server` with
   ...
   a target-bound dry-run that does not mutate Discord.
2. Present its source evidence, blockers, operation summary, risks, and
   `approval_id` for review.
3. After approval, call `guild_blueprint_apply` with the unchanged caller-local
   `plan_ref`, the same IDs and `approval_id`, `__confirm:true`, and an optional
   `operation_budget` from 1 through 50. Portable and older clients may instead
   pass the legacy self-contained `plan_token`, but never pass both.
...
Plan envelopes, checkpoints, and Activity Evidence default to the platform
