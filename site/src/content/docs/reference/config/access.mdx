...
was supplied. This identity lock also authorizes the application-emoji write
surface when `ALLOWED_GUILDS` is active: those tools can affect the bot's
application globally, never another application. See [Credential
lifecycle](/discord-mcp/operations/credentials/).
...
guild returns `GUILD_NOT_ALLOWED`. The application-emoji write tools are a
