...
- **Use:** Stores authenticated append-only checkpoints and apply locks for `guild_blueprint_apply`.
- **Example:** `MCP_BLUEPRINT_STATE_DIR=/var/lib/discord-mcp/blueprints`

Use a private persistent directory when running in a container. The checkpoint
contains Discord target IDs and resolved bindings, but never the bot token or
`plan_token`. A Discord bot-token rotation invalidates pending checkpoints, and
an HTTP access-token rotation invalidates plans created through that HTTP caller
boundary. Create a fresh `guild_blueprint_plan` after either rotation. See
...
`full` advertises the existing 207-tool catalog for maximum client
