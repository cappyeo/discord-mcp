...
- **Default:** `true`
- **Use:** Retries selected surfaced transient failures when replay is safe.
  `@discordjs/rest` separately owns its route queue and 429 waits.

### `MCP_RETRY_MAX_ATTEMPTS`

- **Type:** integer from 1 to 10
- **Default:** `3`
- **Use:** Number of extra retries after the initial call. The default permits
  up to four calls. Set `MCP_RETRY_ENABLED=false` to disable retry.
...
- **Default:** `10000`
- **Use:** Caps exponential backoff and the known Discord `Retry-After` that
  may be absorbed inside one MCP request. Longer rate-limit windows surface
  immediately so resumable callers can checkpoint and wait outside the server
  process instead of exceeding their request deadline.
...
The pipeline executes sequentially and its parent holds no REST slot, so a
limit of 1 is valid (though it serializes all Discord REST work).
