...
OpenTelemetry SDK is not imported.
...
- **Type:** URL
- **Default:** unset
- **Use:** OTLP collector endpoint. A console exporter can still run without one.
...
- **Use:** Selects the OTLP wire format; `http/protobuf` is the recommended default.
...
- **Type:** boolean string
- **Default:** `false`
- **Use:** Writes spans to stderr for local debugging when no collector is available.
