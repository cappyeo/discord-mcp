...
For PR-level history, see the [GitHub releases](https://github.com/cappyeo/discord-mcp/releases)
...
- `@discordjs/rest`, `@modelcontextprotocol/sdk`, `zod` and `@sapphire/pieces`
  moved from `dependencies` to `peerDependencies` on `@cappyeo/discord-mcp-core`. Each
  crosses the public `.d.ts` surface nominally, so a consumer with a duplicate
  copy could not call `buildServer` at all. `@cappyeo/discord-mcp` users are
  unaffected.
...
909 → 1005, with a coverage gate on `@cappyeo/discord-mcp-core` (98.9% statements) and
...
- `@cappyeo/discord-mcp-server-mocks` workspace package — eliminates 165
  relative-path imports across the test tree; consumers now `import from
  '@cappyeo/discord-mcp-server-mocks'`.
...
- npm publish dry-run verified for `@cappyeo/discord-mcp-core` and
  `@cappyeo/discord-mcp` — `prepack` ships root README + LICENSE in both
...
- `@cappyeo/discord-mcp-core` and `@cappyeo/discord-mcp` packages.
