...
- `def.idempotent` is runtime metadata used by telemetry and audit correlation.
  It is not synonymous with read-only: an idempotent PUT can write. The audit
  layer skips only tools whose `annotations.readOnlyHint` is explicitly true.
