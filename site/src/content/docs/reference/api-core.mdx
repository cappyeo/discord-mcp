---
title: "@cappyeo/discord-mcp-core API"
description: Public TypeScript exports from @cappyeo/discord-mcp-core. defineTool, buildServer, loadConfig, wrapRestWithResilience, error classes, branded snowflake types.
sidebar:
  order: 3
---
...
`@cappyeo/discord-mcp-core` is the embeddable engine. The `discord-mcp` binary is one
...
The authoritative public surface is **everything re-exported from
[`packages/mcp-core/src/index.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/index.ts)**.
That barrel is the contract v1.0.0 freezes.

This page documents the most commonly used of those exports with prose and
examples. It is a guided tour, not an inventory: **absence from this page does
not make an export private.** Roughly half the barrel — stores, middleware,
capability router, pipeline internals, telemetry conventions, pagination and
response helpers — is public and stable without having a section here. Read
the barrel for the full list.

What *is* private: **deep imports into subpaths**. Importing from
`@cappyeo/discord-mcp-core/dist/tools/_lib/...` or any path other than the package
root is unsupported and may break in any release, including a patch.
...
| `def.annotations`| `ToolAnnotations`                                                               | MCP tool annotations. The four keys are `readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint` — all optional booleans. |
| `def.idempotent` | `boolean` (default `false`)                                                     | Carried into `MiddlewareContext.tool`, emitted as the `mcp.tool.idempotent` telemetry attribute, and used by `auditMiddleware` to **skip** audit emission (`true` = read-only, no audit event). It does **not** drive retry — retry lives in the cockatiel REST policy and is keyed on error classification, not on this flag. |
| `def.handler`    | `(args, ctx: ToolRunContext) => Promise<O>`                                     | The actual implementation. Receives parsed args plus a context carrying `signal`.                |
...
```ts
import { ChannelId, defineTool } from '@cappyeo/discord-mcp-core';
import { z } from 'zod';
...
export default defineTool({
  ...
  async handler({ channel_id }, ctx) {
    // ctx carries `signal` and nothing else. Pass it to anything abortable.
    const res = await fetch(`https://example.internal/channels/${channel_id}`, {
      signal: ctx.signal,
    });
    const ch = (await res.json()) as { name: string };
    return { summary: `#${ch.name}` };
  },
});
```

<Aside type="note">
The handler context is `ToolRunContext`, which is `{ signal: AbortSignal }` —
that is the whole interface. There is no `ctx.invoke` and no sampling handle
on it. Tool-to-tool calls go through `mcp_pipeline` at the protocol level
rather than through a handler-side helper.
</Aside>

Source: [`packages/mcp-core/src/tools/_lib/defineTool.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/tools/_lib/defineTool.ts).
...
transport can flush it via `shutdown()` on `SIGTERM`.

**Example**:

```ts
import { REST } from '@discordjs/rest';
import { buildServer, loadConfig, wrapRestWithResilience, buildPolicy } from '@cappyeo/discord-mcp-core';
...
<Aside type="danger">
The method is **`shutdown()`**, not `close()`. `AuditSink` declares
`shutdown?(): Promise<void>` and nothing named `close`.

Because the call site is optional-chained, `auditSink.close?.()` does not
throw — it resolves to `undefined` and the process exits immediately. The
result is that `FileAuditSink` never flushes: every audit record still sitting
in the write stream's buffer is lost on **every** shutdown, silently. That
directly breaks the durability guarantee in
[Operations → Audit](/discord-mcp/operations/audit/).

Optional chaining is correct here — `shutdown` is genuinely optional on the
interface, and `StderrAuditSink` does not implement it — but it means a typo
in the method name fails silently rather than loudly. Check the spelling.
</Aside>

Source: [`packages/mcp-core/src/server.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/server.ts).
...
Source: [`packages/mcp-core/src/config.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/config.ts).
...
```ts
import { REST } from '@discordjs/rest';
import { wrapRestWithResilience, buildPolicy, loadConfig, classifyDiscordError } from '@cappyeo/discord-mcp-core';
...
Source: [`packages/mcp-core/src/rest/resilient.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/rest/resilient.ts).
...
| ----------------------------- | --------------------- | --------- | ---------------------------------------------------------------------------------------------- |
| `DiscordRetryableError`       | (internal marker)     | yes       | [`rest/errors.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/rest/errors.ts) |
| `CircuitOpenError`            | `CIRCUIT_OPEN`        | yes       | [`errors/server.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/server.ts) |
| `BulkheadFullError`           | `BULKHEAD_FULL`       | yes       | [`errors/server.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/server.ts) |
...
| `GuildNotAllowedError`        | `GUILD_NOT_ALLOWED`   | no        | `errors/index.ts` — **never thrown, see below**                                                |
...
<Aside type="caution" title="`GUILD_NOT_ALLOWED` is not implemented">
`GuildNotAllowedError` is exported and formatted, but **nothing constructs
it**. There is no guild allowlist at any layer: no `ALLOWED_GUILDS`
environment variable is read by any code path, and no precondition,
middleware, or tool checks a guild id against one. A deployment can call
every tool against every guild the bot is in.

The error's own `recoveryHint` says "Add guild `<id>` to `ALLOWED_GUILDS`
env" — that hint is aspirational and unreachable. Do not build an access
model on it.

Per-guild restriction is **deferred post-1.0**. Today the only real limits
are the bot's own Discord permissions and, for coarse-grained tool gating,
[`MCP_CATEGORIES`](/discord-mcp/reference/config/#mcp_categories).
</Aside>
...
```ts
import { formatErrorForUser, DiscordPermissionError } from '@cappyeo/discord-mcp-core';
...
Source: [`packages/mcp-core/src/errors/format.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/format.ts).
...
```ts
import { ChannelId, MessageId, type ChannelId as ChannelIdT } from '@cappyeo/discord-mcp-core';
...
Source: [`packages/mcp-core/src/tools/_lib/snowflake.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/tools/_lib/snowflake.ts).
