---
title: Architecture
description: How discord-mcp is built - tool registry, policy middleware, resilient REST adapter, optional Gateway, and protocol boundaries.
type: explanation
prev: false
next: false
sidebar:
...
- **Tool piece** implements one of the 209 tools, using Discord REST, MCP
...
<DocsCardGrid
  links={[
    { title: 'Middleware chain', href: '/discord-mcp/architecture/middleware-chain/', description: 'The ordered target, validation, scope, write, confirmation, telemetry, and audit controls wrapping every tool call.' },
    ...
    { title: 'Gateway', href: '/discord-mcp/architecture/gateway/', description: 'The optional --gateway mode: five event URI keys, an allowlisted live guild snapshot, in-memory subscriptions, debounce, and REST-only fallback.' },
    ...
    { title: 'Confirmation', href: '/discord-mcp/architecture/confirmation/', description: 'The payload-bound Components V2 approval and the __confirm:true + MCP_DRY_RUN=false guard for explicitly registered high-risk tools.' },
...
- [Tools](/discord-mcp/tools/) - auto-generated reference for all 209 tools.
