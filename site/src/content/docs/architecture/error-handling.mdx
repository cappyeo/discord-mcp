...
│  ├─ GuildNotAllowedError        (GUILD_NOT_ALLOWED)   ← never constructed
...
Source:
[`errors/base.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/base.ts),
[`errors/client.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/client.ts),
[`errors/server.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/server.ts).

`GuildNotAllowedError` is defined and formatted but **no code path ever
constructs it**. There is no guild allowlist: no `ALLOWED_GUILDS` env var is
read anywhere, and no layer compares a guild id against one. An agent can act
on every guild the bot has joined. Per-guild restriction is **deferred
post-1.0** — the class and its `GUILD_NOT_ALLOWED` code are reserved for that
work. Treat this row as a placeholder, not a control you can rely on.
...
Source: [`errors/format.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/format.ts).
...
```json
{
  ...
  "recovery_hint": "Set MCP_DRY_RUN=false AND pass __confirm:true to actually execute",
...
2. Surfaces the preview to its own human as "this is what I would do, are
   you sure?" and re-issues with `__confirm:true` only once approved. That
   prompt is entirely client-side — the server never initiates one.
...
Mapping happens in [`rest/errors.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/rest/errors.ts)
...
| Concern | File |
| ------- | ---- |
| Base classes | [`errors/base.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/base.ts) |
| Client errors | [`errors/client.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/client.ts) |
| Server errors | [`errors/server.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/server.ts) |
| Wire formatter | [`errors/format.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/errors/format.ts) |
| Cockatiel mappings | [`rest/errors.ts`](https://github.com/cappyeo/discord-mcp/blob/main/packages/mcp-core/src/rest/errors.ts) |
