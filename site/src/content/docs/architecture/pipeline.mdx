---
title: Pipeline
description: The mcp_pipeline executor - sequential fail-fast tool calls with variable interpolation and a recursion guard.
sidebar:
  order: 4
---
...
syntax. It exists because some workflows are one operator intent but require
multiple Discord REST calls - e.g. "post the announcement, schedule the
event, then grant the role." Without a pipeline, the agent makes
three round-trips and has to handle partial failures itself; with one, the
server captures intermediate state and propagates failures cleanly. It does
not provide transactions or rollback: completed Discord side effects remain
...
which is then rejected or transformed by the target tool's Zod schema. Name your
...
`PIPELINE_RECURSION` error code. A flat sequence keeps the 20-step bound,
variable namespace, cancellation, and failure reporting unambiguous.
</Aside>
...
1. **Cohesive intent**: "announce + schedule + grant" is one operator action,
...
2. **Single observability unit**: one pipeline = one parent span with leaf tool
   spans. Audit emits a parent event because `mcp_pipeline` is non-idempotent,
   plus events for every leaf that is not explicitly read-only. Idempotent
   writes remain auditable; only read-only leaves are omitted from the mutation
   trail.
...
- **Bulkhead saturation**: if a leaf fails with `BULKHEAD_FULL`, the
  pipeline propagates it. The bulkhead is the outermost fast-reject layer, so
  its rejection never enters the inner retry policy; the pipeline itself does
  not retry the step. The caller can back off and start a new call later.
